...
std::vector<Cloud::Ptr> Segmentation::getClusters(
    const IndicesClustersPtr &clusters) {
  std::vector<Cloud::Ptr> segmented_clouds;
  pcl::PointIndicesPtr segmented_indices(new pcl::PointIndices);
  int j = 0;
  for (IndicesClusters::const_iterator it = clusters->begin();
       it != clusters->end(); ++it) {
    Cloud::Ptr cloud_cluster(new Cloud);
    for (std::vector<int>::const_iterator pit = it->indices.begin();
         pit != it->indices.end(); pit++) {
      cloud_cluster->points.push_back(cloud_->points[*pit]);
      segmented_indices->indices.push_back(*pit);
    }
...
std::vector<Cloud::Ptr> Segmentation::getClusters(
    const pcl::PointIndicesPtr &clusters) {
  std::vector<Cloud::Ptr> segmented_clouds;
...
void Segmentation::ConditionalEuclideanClustering(ConditionFunction func,
  ...
  emit segmentationResult(getClusters(clusters), time.toc());
}

void Segmentation::EuclideanClusterExtraction(double tolerance,
                                              int min_cluster_size,
                                              int max_cluster_size) {
  pcl::console::TicToc time;
  time.tic();
...
void Segmentation::ExtractPolygonalPrismData(const Cloud::Ptr &hull,
                                             double height_min,
                                             double height_max, float vpx,
                                             float vpy, float vpz) {
  pcl::console::TicToc time;
  time.tic();
  pcl::search::KdTree<PointXYZRGBN>::Ptr tree(
  ...
  seg.segment(*indices);
  emit segmentationResult(getClusters(indices), time.toc());
}

void Segmentation::RegionGrowing(int min_cluster_size, int max_cluster_size,
                                 bool smooth_mode, bool curvature_test,
                                 bool residual_test, float smoothness_threshold,
  ...
  reg.setSmoothnessThreshold(deg2rad(smoothness_threshold));
...
void Segmentation::RegionGrowingRGB(
    int min_cluster_size, int max_cluster_size, bool smooth_mode,
...
void Segmentation::SACSegmentation(int model, int method, double threshold,
                                   int max_iterations, double probability,
                                   bool optimize, double min_radius,
                                   double max_radius, const Eigen::Vector3f &ax,
                                   double ea) {
  pcl::console::TicToc time;
  time.tic();
  pcl::search::KdTree<PointXYZRGBN>::Ptr tree(
  ...
  return segmentationResult(getClusters(indices), cofes, time.toc());
}

void Segmentation::SACSegmentationFromNormals(
    int model, int method, double threshold, int max_iterations,
...
void Segmentation::SeededHueSegmentation(double tolerance, float delta_hue) {
  ...
  emit segmentationResult(getClusters(indices_out), time.toc());
}
...
void Segmentation::SegmentDifferences(const Cloud::Ptr &tar_cloud,
  ...
  emit segmentationResult(segmented_clouds, time.toc());
}

void Segmentation::SupervoxelClustering(
