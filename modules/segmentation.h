#ifndef PCA_SEGMENTATION_H
#define PCA_SEGMENTATION_H
...
using ConditionFunction =
    std::function<bool(const PointXYZRGBN &, const PointXYZRGBN &, float)>;

class PCA_EXPORT Segmentation : public QObject {
  Q_OBJECT

 public:
  explicit Segmentation(QObject *parent = nullptr)
      : QObject(parent), cloud_(), negative_(false) {}
 ...
 private:
  ...
  std::vector<Cloud::Ptr> getClusters(const pcl::PointIndicesPtr &clusters);

 signals:

  void segmentationResult(const std::vector<Cloud::Ptr> &cloud, float time);

  void segmentationResult(const std::vector<Cloud::Ptr> &cloud,
                          const pcl::ModelCoefficients::Ptr &cofe, float time);
 ...
 public slots:
   ...
   */
  void ConditionalEuclideanClustering(ConditionFunction func,
   ...
   * @param max_cluster_size 设置集群需要包含的最大点数才能被视为有效
   */
  void EuclideanClusterExtraction(double tolerance, int min_cluster_size,
                                  int max_cluster_size);
   ...
   */
  void ExtractPolygonalPrismData(const Cloud::Ptr &hull, double height_min,
                                 double height_max, float vpx, float vpy,
                                 float vpz);
   ...
   */
  void RegionGrowing(int min_cluster_size, int max_cluster_size,
   ...
   */
  void RegionGrowingRGB(int min_cluster_size, int max_cluster_size,
                        bool smooth_mode, bool curvature_test,
                        bool residual_test, float smoothness_threshold,
                        float residual_threshold, float curvature_threshold,
                        int neighbours, float pt_thresh, float re_thresh,
   ...
   */
  void SACSegmentation(int model, int method, double threshold,
                       int max_iterations, double probability, bool optimize,
                       double min_radius, double max_radius,
                       const Eigen::Vector3f &ax, double ea);
   ...
   */
  void SupervoxelClustering(float voxel_resolution, float seed_resolution,
