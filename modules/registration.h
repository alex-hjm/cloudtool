#ifndef PCA_REGISTRATION_H
#define PCA_REGISTRATION_H
...
namespace pca {
typedef pcl::registration::TransformationEstimation<PointXYZRGBN, PointXYZRGBN,
                                                    float>
    TransEst; /*表示转换估计方法的类*/
typedef pcl::registration::CorrespondenceEstimationBase<PointXYZRGBN,
                                                        PointXYZRGBN, float>
    CorreEst; /*表示对应估计方法的类*/
typedef pcl::registration::CorrespondenceRejector
    CorreRej; /*表示对应拒绝方法的类*/

class PCA_EXPORT Registration : public QObject {
  Q_OBJECT
 ...
 signals:
  ...
  void registrationResult(bool success, const Cloud::Ptr &ail_cloud,
                          double score, const Eigen::Matrix4f &matrix,
                          float time);

  void correspondenceEstimationResult(const pcl::CorrespondencesPtr &corr,
                                      float time);

  void correspondenceRejectorResult(const pcl::CorrespondencesPtr &corr, float time);

  void transformationEstimationResult(const Eigen::Matrix4f &matrix,
                                      float time);
 ...
 public:
   ...
   * @param thresh_src thresh_tar
   * 任何高于此距离阈值的特征对应都将被视为不良并被过滤掉
   */
  template <typename Feature>
  void CorrespondenceRejectorFeatures(
      const typename pcl::PointCloud<Feature>::Ptr &source,
      const typename pcl::PointCloud<Feature>::Ptr &target, double thresh_src,
      double thresh_tar) {
    TicToc time;
    time.tic();
   ...
   */
  template <typename Feature>
  void SampleConsensusInitialAlignment(
      const typename pcl::PointCloud<Feature>::Ptr &source,
    ...
    pcl::SampleConsensusInitialAlignment<PointXYZRGBN, PointXYZRGBN, Feature>
        reg;
    ...
    emit registrationResult(
        reg.hasConverged(), ail_cloud, reg.getFitnessScore(),
   ...
   * @param nr_samples 设置每次迭代期间要使用的样本数
   * @param k 设置选择随机特征对应时要使用的邻居数
   * @param similarity_threshold 在 [0,1]
   * 中设置底层多边形对应拒绝器对象的边缘长度之间的相似度阈值，其中 1
   * 是完美匹配。
   * @param inlier_fraction 设置所需的内部分数（输入的）
   */
  template <typename Feature>
  void SampleConsensusPrerejective(
      const typename pcl::PointCloud<Feature>::Ptr &source,
      const typename pcl::PointCloud<Feature>::Ptr &target, int nr_samples, int k,
 ...
 public slots:
   ...
   */
  void CorrespondenceEstimationOrganizedProjection(
  ...
  /**
   * @brief
   * 实现了一种对应拒绝方法，该方法利用输入对应关系在每个模型上形成用户可指定
   *        基数的虚拟多边形，从而利用两个点集之间的低级和姿势不变几何约束。
   * @param cardinality 设置多边形基数
   * @param similarity_threshold 在 [0,1[ 中设置边缘长度之间的相似度阈值，其中 1
   * 是完美匹配
   * @param iterations 设置迭代次数
   */
  void CorrespondenceRejectorPoly(int cardinality, float similarity_threshold,
                                  int iterations);
   ...
   */
  void CorrespondenceRejectorSampleConsensus(double threshold,
                                             int max_iterations, bool refine);
   ...
   */
  void GeneralizedIterativeClosestPoint(int k, int max, double tra_tolerance,
   ...
   */
  void IterativeClosestPointWithNormals(bool use_recip_corre,
   ...
   */
  void NormalDistributionsTransform(float resolution, double step_size,
   ...
   */
  void TransformationEstimationSymmetricPointToPlaneLLS(
      bool enforce_same_direction_normals);
