...
class CT_EXPORT CloudView : public QVTKOpenGLNativeWidget {
 ...
 public:
   ...
   * @brief 更新相机参数
   */
  void updateCamera() { viewer_->updateCamera(); }
  ...
  void resetCameraViewpoint(const QString& id) {
    viewer_->resetCameraViewpoint(id.toStdString());
  }

  /**
   * @brief 获取当前视角位姿
   */
  Eigen::Affine3f getviewer_Pose() { return viewer_->getViewerPose(); }

  /**
   * @brief 初始化相机参数
   */
  void initCameraParameters() { viewer_->initCameraParameters(); }

  /**
   * @brief 导入相机参数
   */
  void loadCameraParameters(const QString& file) {
    viewer_->loadCameraParameters(file.toStdString());
  }
  ...
  void setCameraParameters(const Eigen::Matrix3f& intrinsics,
    ...
    viewer_->setCameraParameters(intrinsics, extrinsics);
  }

  /**
   * @brief 设置相机剪切距离
   * @param _near 近剪裁距离（不会绘制比这更接近相机的物体）
   * @param _far 远剪裁距离（不会绘制比这更远的物体到相机）
   */
  void setCameraClipDistances(double _near, double _far) {
    viewer_->setCameraClipDistances(_near, _far);
  }
  ...
  void setCameraFieldOfView(double fovy) {
    viewer_->setCameraFieldOfView(fovy);
  }
  /**
   * @brief 将相机参数作为 .cam 文件保存到磁盘。
   */
  void saveCameraParameters(const QString& file) {
    viewer_->saveCameraParameters(file.toStdString());
  }
   ...
   * @brief 设置是否显示帧率
   */
  void setShowFPS(const bool& enable) { viewer_->setShowFPS(enable); }
  ...
  bool contains(const QString& id) {
    return viewer_->contains(id.toStdString());
  }
  ...
  void saveScreenshot(const QString& file) {
    viewer_->saveScreenshot(file.toStdString());
  }
   ...
   */
  void setTopview() {
    viewer_->setCameraPosition(0, 0, 0, 0, -1, 0, 0, 0, -1);
    viewer_->getRenderWindow()->Render();
  }
  ...
  void setLeftSideview() {
    viewer_->setCameraPosition(0, 0, 0, 1, 0, 0, 0, 1, 0);
    viewer_->getRenderWindow()->Render();
  }
