...
namespace ct
{
  ...
  void CloudTree::removeAllClouds()
  {
    for (auto i : m_cloud_vec)
      for (auto j : i) emit removedCloudId(j->id());
  ...
  void CloudTree::setCloudChecked(const Cloud::Ptr& cloud, bool checked)
  {
    if (cloud == nullptr) return;
    Index i = index(cloud->id());
    if (checked && (item(i)->checkState(0) == Qt::Checked)) return;
    if ((!checked) && (item(i)->checkState(0) == Qt::Unchecked)) return;
  ...
  void CloudTree::itemSelectionChangedEvent()
  {
    ...
    for (auto& i : all)
    {
      ...
      Cloud::Ptr cloud = getCloud(i);
      if (it != indexs.end()) m_cloudview->addBox(cloud);
      else m_cloudview->removeShape(cloud->boxId());
    }
    // update table
    ...
    else
    {
      ...
      scale->setValue(0.01);
      if (update_cloud->hasNormals())show_normals->setEnabled(true);
      else show_normals->setEnabled(false);
