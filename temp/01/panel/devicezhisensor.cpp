...
void DeviceZhisensor::init(Console* &co,CloudView* &cv,CloudTree* &ct,ImageTree *&it,ImageView *&iv)
{
...
void DeviceZhisensor::output()
{
    ...
    if(output_type==0) {
        ...
        cloud_view->addCloud(captured_cloud,"zhisensor");
        if(ui->check_withoutcolor->isChecked())
            cloud_view->setCloudColor(captured_cloud,"zhisensor","z");
