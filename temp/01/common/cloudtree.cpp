...
{
    QString path="E:/1_QT_Project/CloudTool/resource/clouds/rabbit.pcd";
...
bool CloudTree::insertCloud(int row,const Cloud::Ptr &cloud, bool selected)
{
    ...
    if(cloud_view->contains(cloud->id)) {
...
void CloudTree::clearCloud(const Index &index)
{
    ...
    cloud_view->removeCloud(cloud->normals_id);
...
void CloudTree::updatePropertiesTable()
{
    ...
    if(selectedClouds.empty()) {
        ...
        cloud_table->removeCellWidget(6,1);
        cloud_view->showId("");
    } else {
        int total_size=0;
        for(auto &i:selectedClouds) {
        ...
        cloud_table->setCellWidget(6,1,show_normals);
        if(selectedClouds.size()>1) {
            id="multi clouds";
            type="undefined";
            ...
            if(!selectedClouds.front()->has_normals)
                show_normals->setEnabled(false);
        ...
        connect(show_normals,&QCheckBox::stateChanged,[=](int state) {
            std::vector<Cloud::Ptr> selectedClouds=this->getSelectedClouds();
            if(state)
                cloud_view->addCloudNormals(selectedClouds.front(),1,0.01,selectedClouds.front()->normals_id);
            else
                cloud_view->removeShape(selectedClouds.front()->normals_id);
        });
    }
...
void CloudTree::updateScreen(QTreeWidgetItem * item, int)
{
    ...
    if(indexs.size()>1) {
        ...
        if(CheckState==Qt::Unchecked)
            this->setCloudChecked(indexs.front(),false);
        else
            this->setCloudChecked(indexs.front(),true);
    }
}
