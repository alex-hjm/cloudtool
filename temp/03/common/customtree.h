...
protected:
    ...
    template <typename T>
    void addItem(int row,const T &data,const QString &parent_id,const QIcon &parent_icon,
                 const bool &selected,std::vector<std::vector<T>> &dataVec)
    {
        if(row==-1) {
            std::vector<T> temp;
            temp.push_back(data);
            ...
            std::vector<T> temp;
            temp.push_back(data);
            dataVec.insert(dataVec.begin()+row,temp);
            QTreeWidgetItem* parent = new QTreeWidgetItem();
            QTreeWidgetItem* child= new QTreeWidgetItem();
            parent->setText(0, parent_id);
            parent->setIcon(0,parent_icon);
