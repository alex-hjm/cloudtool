...
{
public:
    Eigen::Affine3f AffineFromXYZEuler(float x, float y, float z, float roll, float pitch, float yaw)
    {
    ...
    Eigen::Affine3f AffineFromAxisAngle(float angle, float axisX, float axisY,float axisZ,
    ...
    Eigen::Affine3f AffineFromQString(const QString &text,bool &success)
    {
    ...
    Eigen::Matrix3f RotMatrixFromEulerAngle(float roll, float pitch, float yaw)
    {
    ...
    Eigen::Matrix4f MatrixFromXYZEulerAngle(float x, float y, float z,float roll, float pitch, float yaw)
    {
        Eigen::Affine3f rolation=this->AffineFromXYZEuler(x,y,z,roll,pitch,yaw);
        return rolation.matrix();
    }

    Eigen::Matrix4f MatrixFromXYZRotMatrix(float x, float y, float z, Eigen::Matrix3f &rolation)
    {
        Eigen::Matrix4f Martrix=Eigen::Matrix4f::Identity();
    ...
    Eigen::Matrix4f MatrixFromQString(const QString &text,bool &success)
    {
        Eigen::Affine3f rolation=this->AffineFromQString(text,success);
        return rolation.matrix();
    }

    QString QStringFromMatrix(Eigen::MatrixXf Martrix,int decimals)
    {
    ...
    void ConvertHSVtoRGB(float h, float s, float v, float& out_r, float& out_g, float& out_b)
    {
        if (s == 0.0f)
        {
            out_r = out_g = out_b = v;
